...
impl ARGlasses for DefaultLogger {
...
#[test]
fn default_packet_logging_is_a_noop() {
    let mut glasses = DefaultLogger;
    assert!(glasses
        .start_packet_logging(Path::new("this-path-is-never-opened"))
        .is_ok());
    assert!(glasses.stop_packet_logging().is_ok());
}
