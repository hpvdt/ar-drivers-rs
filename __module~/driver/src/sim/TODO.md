...
- [ ] Generate a non-planar attitude trajectory
  ...
  - **Recommended fix:** Change the body-rate direction over time with a smooth deterministic maneuver or seeded multi-axis excitation schedule that visits substantially different roll, pitch, and yaw orientations.
...
- [ ] Balance default angular motion against magnetometer noise
  ...
  - **Description:** Defaults combine a maximum of 1 RPM per body axis, 20 ms between magnetometer samples, a 50
    ...
    magnetometer sample.
  - **Recommended fix:** Provide a calibration-oriented default or selectable profile with faster multi-axis motion, lower magnetometer noise, or both.
...
- [ ] Separate static-calibration and drifting-bias simulator profiles
  ...
  - **Recommended fix:** Make the baseline calibration profile use zero hard-iron drift and retain the current drift as a separately selected adaptive-calibration stress profile.
...
- [ ] Model finite magnetometer range instead of unbounded output
  ...
  - **Recommended fix:** Add a configurable physical magnetometer range and clamp readings or report saturation when the distorted noisy value exceeds it.
